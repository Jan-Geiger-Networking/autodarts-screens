...
import type { MatchState, MatchEvent, Player, PlayerScore } from '../../shared/typen'
...
const score = (playerId: string, ueberschreibungen: Partial<PlayerScore> = {}): PlayerScore => ({
  playerId,
  remaining: 301,
  legs: 0,
  sets: 0,
...
describe('szeneAusZustand', () => {
  ...
  it('zeigt idle wenn keine Spieler bekannt sind, unabhaengig von phase', () => {
    const ergebnis = szeneAusZustand(zustand({ phase: 'playing', players: [] }), -1)
    expect(ergebnis.basis).toBe('idle')
  })
  ...
  it('playerChange loest die passende Ueberlagerung aus', () => {
    const ereignis: MatchEvent = { seq: 3, kind: 'playerChange', toPlayerId: 'p2' }
    const ergebnis = szeneAusZustand(zustand({ lastEvent: ereignis }), 2)
    expect(ergebnis.ueberlagerung).toEqual({ art: 'playerChange', seq: 3, zuSpielerId: 'p2' })
  ...
  it('oneEighty loest bigMoment aus', () => {
    ...
    expect(ergebnis.ueberlagerung).toEqual({ art: 'bigMoment', seq: 7, anlass: 'oneEighty', spielerId: 'p1' })
  })

  it('highFinish loest bigMoment aus', () => {
    const ereignis: MatchEvent = { seq: 8, kind: 'highFinish', playerId: 'p2', score: 141 }
    const ergebnis = szeneAusZustand(zustand({ lastEvent: ereignis }), 7)
    expect(ergebnis.ueberlagerung).toEqual({ art: 'bigMoment', seq: 8, anlass: 'highFinish', spielerId: 'p2' })
  })
  ...
  it('legWon loest legWin aus', () => {
    ...
    expect(ergebnis.ueberlagerung).toEqual({ art: 'legWin', seq: 9, spielerId: 'p1' })
  })
  ...
  it('matchWon loest matchWin aus', () => {
    ...
    expect(ergebnis.ueberlagerung).toEqual({ art: 'matchWin', seq: 10, spielerId: 'p1' })
  })
